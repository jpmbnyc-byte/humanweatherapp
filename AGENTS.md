...
- **Dev server** (`bun run dev`) binds to `http://localhost:8080` (host/port set in `vite.config.ts`, not the Vite default 5173). The build is a native `vite.config.ts` (TanStack Start + React + Tailwind v4 + nitro `cloudflare-module`); it also injects `COOP`/`COEP` (`credentialless`) via middleware for cross-origin isolation, which the in-browser Kokoro voice engine needs for SharedArrayBuffer.
...
- **Voice (The Tender tab)**: in-browser **Kokoro** TTS via `kokoro-js` (loaded from jsDelivr, model from Hugging Face), lazy-loaded on first "Listen" and cached for offline use. No API key or server route. It requires cross-origin isolation (the COOP/COEP headers above) for SharedArrayBuffer; note the ~86MB model is heavy and needs a capable device (ideally WebGPU) to run in real time.
